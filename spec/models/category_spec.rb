...
RSpec.describe Category, type: :model do
  ...
  it "is valid with valid attributes" do
    category = Category.new(name: "Food", icon: "https://example.com/icon.png", user: user)
    expect(category).to be_valid
  end

  it "is not valid without a name" do
    category = Category.new(name: nil, icon: "https://example.com/icon.png", user: user)
  ...
  it "is not valid if name length exceeds 250 characters" do
    category = Category.new(name: "a" * 251, icon: "https://example.com/icon.png", user: user)
    expect(category).to_not be_valid
  end
