...
RSpec.describe Expense, type: :model do
  ...
  it 'is valid with valid attributes' do
    expense = Expense.new(name: 'Test Expense', amount: 100, category:)
    expect(expense).to be_valid
  end

  it 'is not valid without a name' do
    expense = Expense.new(amount: 100, category:)
  ...
  it 'is not valid if amount is not a number' do
    expense = Expense.new(name: 'Test Expense', amount: 'invalid', category:)
    expect(expense).not_to be_valid
  end
  ...
  it 'is not valid if amount is greater than 10,000' do
    expense = Expense.new(name: 'Test Expense', amount: 20_000, category:)
    expect(expense).not_to be_valid
  end
